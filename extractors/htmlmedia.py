...
def _quality_probe_text(url):
    """[PATCH 94] What quality labelling may look at: the lowercased URL PATH only.
    Not the host, query, #fragment or |piped headers: '?title=hd1080', a token
    containing '4k' or 'a_h/', or a Referer with '/1080/' in it used to change
    the label."""
    try:
        base = _correct_stream_url((url or "").split("|", 1)[0])
        return urlparse(base).path.lower()
    except Exception:
        return ""
...
def _label_quality_variant(url):
    ...
    m = re.search(r'(?<![a-z0-9])(2160|1080|720|480|360|240)p?(?![a-z0-9])', lower)
    if m:
        return m.group(1) + "p"
    m = re.search(r'(?<![a-z0-9])(4k|fhd)(?![a-z0-9])', lower)
    if m:
        return "2160p" if m.group(1) == "4k" else "1080p"
    return None
